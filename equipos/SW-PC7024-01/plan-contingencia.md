# 🚨 SW-PC7024-01 — Plan de contingencia
...
> ⚠️ Falta la versión de firmware y el nombre del `.stk`: ejecuta **`show version`** (el `show system` que se capturó no los incluye).
...
```text
show running-config
copy running-config tftp://192.168.104.10/SW-PC7024-01_AAAA-MM-DD.cfg
```

También por FTP:

```text
copy running-config ftp://labftp:<clave>@192.168.104.10/imagenes/SW-PC7024-01_AAAA-MM-DD.cfg
```
...
```text
copy ?
copy image1 ftp://labftp:<clave>@192.168.104.10/imagenes/SW-PC7024-01_PC7024.stk
```

Restaurar configuración:

```text
copy tftp://192.168.104.10/SW-PC7024-01_base.cfg startup-config
reload
```
...
````markdown
### Intervención — SW-PC7024-01 — AAAA-MM-DD
