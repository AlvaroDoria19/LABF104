# 🚨 FW-SRX320-01 — Plan de contingencia
...
```text
configure
load override /var/tmp/FW-SRX320-01_base.conf
commit confirmed 5
commit
exit
request system snapshot
...
Copia la salida a `backups/configs/FW-SRX320-01_AAAA-MM-DD.conf`.

**Traer el fichero a la PC del laboratorio** con `scp` desde Windows:

```text
scp lab-admin@192.168.104.51:/config/juniper.conf.gz C:\LabF104\imagenes\FW-SRX320-01_juniper.conf.gz
```
...
cp /config/juniper.conf.gz /var/tmp/usb/FW-SRX320-01_juniper.conf.gz
ls -lh /var/tmp/usb
umount /var/tmp/usb
exit
```
...
````markdown
### Intervención — FW-SRX320-01 — AAAA-MM-DD
