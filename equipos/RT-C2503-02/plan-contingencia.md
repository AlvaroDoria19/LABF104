...
|---|---|:--:|
| ☁️ **Google Drive del laboratorio** | [IMAGENES DEL ROUTER](https://drive.google.com/drive/folders/13WPTIbHg_Qm4KvmR9UxTH6fvjwcnqQHr?usp=sharing) | ☐ |
...
```text
PC-1 (TFTP)                    RT-C2620-01                          RT-C2503-02
192.168.104.10 ── LAN ──  Fa0/0 192.168.104.43
                          Serial0/0 172.16.104.6/30 ══serie══ Serial0 172.16.104.5/30
```
...
configure terminal
 interface FastEthernet0/0
  ...
  exit
 interface Serial0/0
  ip address 172.16.104.6 255.255.255.252
  clock rate 2000000
  no shutdown
 end
write memory
...
**Paso 3 — En el RT-C2503-02, entra a RXBOOT y configura el `Serial0`** en vez de `Ethernet0`:
...
Router(boot)(config-if)#  ip address 172.16.104.5 255.255.255.252
Router(boot)(config-if)#  no shutdown
Router(boot)(config-if)# exit
Router(boot)(config)# ip route 0.0.0.0 0.0.0.0 172.16.104.6
Router(boot)(config)# end
Router(boot)# ping 192.168.104.10
Router(boot)# copy tftp flash
```
...
> `ip default-gateway 172.16.104.6` en su lugar — según la versión exacta de RXBOOT, uno de los
...
**Paso 5 — Limpieza.** Cuando el RT-C2503-02 vuelva a arrancar con su IOS completo, decide:
...
3. El router queda a la espera de recibir el fichero por el puerto de consola. Cambia tu emulador
   de terminal a modo **XMODEM** y envía `igs-inr-l.111-17`:
